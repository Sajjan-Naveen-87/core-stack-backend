...
> For each: the `{Input} → Work → {Output}` flow, then the **LOCAL part** (Python math,
> directly portable to OCaml) and the **GEE part** (cloud calls — portable only by
> reimplementing from the formulas in `formuale.md` + feeding small GEE-exported inputs,
> per the golden-file test method).
>
...
| 12 | `feature/tree_health_pipeline_recompute` | none | 100% GEE | Hard (TB-scale raster source) |
...
- **LOCAL part:** none today.
- **GEE part (→ OCaml):** the math is trivial (clip + per-year mean + the §9 masks in `formuale.md`), **but the input is a TB-scale pan-India raster series** living in GEE. Portable only with per-block exports (fine for testing) — bulk replacement needs the upstream canopy products downloaded from their original providers. Do late.
...
- **GEE part (→ OCaml):** the formulas are deterministic pixel physics — very portable *math* (numpy-style → `owl`/Bigarray). The cost is **data**: multi-GB monthly Landsat+GLDAS stacks per year per region. Per-block testing via GEE exports works; full replacement = serious ingestion pipelines. Do late.
...
- **LOCAL part:** none today.
- **GEE part (→ OCaml):** hardest in the catalog — multi-sensor imagery at hundreds of GB per region-year, plus cloud-mask + regression chains. Port *only* if your research needs a fully GEE-free NDVI source; otherwise treat its output as an input you export.
