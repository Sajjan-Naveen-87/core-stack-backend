...
There are two distinct kinds of "calculation" in the codebase, and the distinction is
**the whole point of this document**:

| Kind                        | Where it runs                          | Libraries                                                                    | OCaml-portable?                             |
| --------------------------- | -------------------------------------- | ---------------------------------------------------------------------------- | ------------------------------------------- |
| **Cloud calculation** | Google Earth Engine (GEE), server-side | `ee` (Earth Engine Python API)                                             | No — it's a thin client to Google's cloud. |
| **Local calculation** | The Python process / Celery worker     | `geopandas`, `shapely`, `fiona`, `rasterio`, `numpy`, `gdal/ogr` | **Yes — this is the target.**        |

The "Python dependency on geoserver calculations" you want to convert is the **local
calculation** layer: the pure-Python geospatial math that produces the vectors/rasters
which are then published to GeoServer.
...
**reimplement the C2 local-calculation modules in OCaml**, keep `geoserver_utils.py`
(or an OCaml equivalent) as the thin publish step, and leave GEE alone (it's a remote
service, not portable code).
...
**Note on `terrain_*`, `lulc_*`, `mws/*`:** in `main` these run *inside GEE* (`ee.Terrain.slope`, `ee.Image.pixelArea`, focal kernels), so they are **not** directly portable — they are cloud calls. They only become local (and portable) on the `feature/local-compute-station` branch (see §5), where they are rewritten as `*_local.py` using rasterio/numpy against locally-downloaded base rasters.
...
**Decision:** base the OCaml extraction on `feature/local-compute-station`'s structure
(`config_loader` + per-module `*_local.py` + `geoserver_utils` as the output seam), and
pull extra algorithms from `feature/local_compute_by_shiv` as needed.

See `convert.md` for the step-by-step conversion procedure.
